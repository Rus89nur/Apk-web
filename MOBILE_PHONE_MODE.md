...
| `js/wizard.js` | Панели `wizard-panel--violations` / `--conclusions`, шаги мастера, сводка |
| `js/wizard-violations-ui.js` | Общий UI шага «Нарушения» для акта и справки (карточки, поиск, фото, DnD) |
| `js/photo-lightbox.js` | Просмотр фото нарушений (lightbox) |
| `js/spravka-wizard.js` | Мастер справки по ПБ (7 шагов) |
| `js/spravka-utils.js` | Модель `spravka`, формат нарушений для Word, организации |
| `js/doc-generator.js` | Генерация Word акта и справки, маркеры шаблонов |
| `js/defaults-bootstrap.js` | Шаблоны Word (акт + справка), пресеты, seed |
...
| `js/app.js` | `GAZPROM_WEB_BUILD`, `syncAppBuildLabel()`, `isNewerBuild` / force refresh (web-254) |
| `js/elimination-editor.js` | Карточки актов на экране **Устранение** (`#eliminationCardList`) |
| `js/catalog-editor.js` | Справочники комиссии/орг./объектов/представителей (fullscreen на телефоне, web-243) |
| `js/reports-dashboard.js` | Дашборд отчётов; диаграмма подрядчиков с фильтрами (web-253) |
| `sw.js` | `CACHE_NAME` = `gazprom-web-vXX` |
| `.github/workflows/deploy-gazprom-web.yml` | Pages: `checkout ref: main`, `cancel-in-progress: true` (web-254) |
...
- **`.main`**: единственный вертикальный скролл. На телефоне `display: block` (не flex-колонка) — иначе `.screen.active` с `flex: 1; min-height: 0` сжимался под высоту вьюпорта, низ уезжал под бар, и прокрутки не хватало (web-251).
- **Отступ над баром (web-255):** `padding-bottom` только у **активного** `.main > .screen` = высота `.bottom-nav` до низа экрана **+ 8px**. Спейсер `.gazprom-scroll-bottom-spacer` **высота 0**. Не дублировать отступ на `.main`, host-списках и Safari inset. Кнопки «Назад/Далее» **в потоке** (`position: static`), не `fixed`.
...
--gazprom-nav-block: JS `computeNavBlockPx()` = `max(nav.height, innerHeight - nav.top) + 8px` (web-255; раньше +48px и min 120 — слишком большой зазор).
...
- **web-252:** поиск нарушений в акте — **одна строка** (поле + кнопки `flex-wrap: nowrap`, поле `flex: 1 1 auto`).
...
**Остальные модалки** (`vr-form-overlay`, confirm): по-прежнему **90%** ширины (`--mobile-modal-width`), центрирование flex, на корне `touch-action: none`. Справочники комиссии/орг./объектов/представителей на телефоне — **весь экран** (§3.15, web-243).
...
- **web-250:** `#wVyvody` **не** `readonly` после вставки шаблона — текст сразу правится, копируется, удаляется; курсор в конец поля.
...
### 3.7. Кнопки «Назад» / «Далее» (web-66, web-68, web-71, web-242→246, web-251, web-255)
...
- **web-242:** временно `position: fixed` над `.bottom-nav` — пользователь отверг (кнопки не должны «прилипать»).
- **web-246:** снова **в потоке** (`position: static`), едут вместе со страницей.
- **web-251:** экран не сжимается под вьюпорт — можно долистать футер **выше** бара.
- **web-255:** зазор над баром = высота панели + 8px (без второго спейсера и запаса 48+24px).
...
### 3.14. Мастер справки по ПБ (web-217, web-219, web-221, web-222)

Модуль: **`js/spravka-wizard.js`**, модель — **`js/spravka-utils.js`**. Экран: **Справка** в навигации.
...
|-----|------------|
| 0 | Дата справки |
| 1 | Объекты проверки (чипы из каталога) |
| 2 | Детали объектов (код, линия ГП и т.д.) |
| 3 | Субподрядчики (чипы из справочника организаций → `subcontractorsList` для Word) |
| 4 | Нарушения — общий UI `wizard-violations-ui.js` (см. §3.3) |
| 5 | Работники — таблица: организация только из каталога (`orgId` + `orgName`), числовые поля ПБ/работников |
| 6 | Замечания, заключение, генерация Word |

**web-217 — объекты в справке:** выбор объектов из каталога (чипы), карточки деталей на шаге 2, стабильный импорт бэкапа.

**web-219 — кроссбраузерность:** улучшения file picker на мобильных, стабильность выбора файлов.

**web-221 — шаблон Word и UI нарушений:**

- **Настройки → Шаблоны Word** — раздел **«Справка по ПБ»** рядом с шаблоном акта.
- Шаблон по умолчанию: `assets/defaults/Шаблон_справки_ПБ.docx`.
- Загрузка своего `.docx`, список сохранённых шаблонов, скачивание пустого шаблона с маркерами (`SPRAVKA_MARKER_GUIDE` в `doc-generator.js`).
- Плитка настроек: статус `акт` / `справка` / `акт+справка` (`ui-bindings.js`).
- Бэкап/импорт: `spravkaTemplate`, `spravkaTemplateName`, `savedSpravkaTemplates` (`backup-import.js`, `catalog-service.js`).
- **Общий UI нарушений** вынесен в `wizard-violations-ui.js` — подключён в шаг 3 акта (`wizard.js`) и шаг 4 справки (`spravka-wizard.js`): карточки, поиск, миниатюры фото, drag-and-drop, секция «Все фото», lightbox (`photo-lightbox.js`).

**web-222 — формат текста нарушений в Word:**

На шаге 4 справки — панель «Как нарушения попадут в текст Word-документа» с переключателями (кнопки `btn-org-filter`):

| Переключатель | Поле в черновике | Эффект |
|---|---|---|
| **Место нарушения** | `violationFormat.includeMesto` | Префикс из `mesto`: `Котельная №2: …` |
| **Пункт правил (в скобках)** | `violationFormat.includeRuleRef` | Суффикс из `urlToPravilo` (или `formulaFromRules`): `… (п. 4.1 СП …)` |

Оба включены по умолчанию; состояние в `draft.violationFormat`, autosave.

**Пример:** `Котельная №2: Не проведён инструктаж (п. 4.1 СП 12-135-2003)`

В `buildSpravkaTemplateData()` маркеры `#ddescrVi#` / `#ddescpitVi#` — через `SpravkaUtils.formatViolationText()`. Отдельные поля `TitleViolatation`, `urlDoc`, `formula` заполняются для шаблонов с раздельными маркерами.

Карточки нарушений на шаге справки показывают тот же формат; дублирующие строки 📍 / 📄 скрываются, если уже в заголовке.

**Основные маркеры справки:**
...
### 3.15. Справочники на телефоне (web-243, web-246)

Экраны комиссии / организаций / объектов / представителей (`catalog-editor.js`):

- Список — **на весь экран карточками**, не узкая таблица десктопа.
- Шаги мастера со справочниками: длинные названия **в колонку**, без горизонтального обрезания.
- **web-246:** кнопки **«Добавить»** и **«Отмена»** в формах справочников — **одинаковый размер** (равная высота/ширина ряда).

### 3.16. Главная (web-241, web-244)

- **web-241:** вместо длинной подписи на титуле — живая статистика нарушений (объём / устранено / просрочка).
- **web-244:** убрана лишняя строка сразу под приветствием «Добрый день!…». Карточки «Текущий номер / Актов в работе / Черновиков» на месте. Метка сборки — внизу `#appBuildId`.

### 3.17. Обновление PWA без отката (web-254)

**Проблема 26.08.2026:** «Настройки → Обновить приложение» скачивало то, что лежало на Pages. Задержанные деплои старых SHA перезаписали сайт (253→**244**), а клиент считал любую **другую** цифру обновлением.

**Исправление:**

| Слой | Правило |
|------|---------|
| Workflow | Всегда `checkout ref: main`; `cancel-in-progress: true` |
| Клиент | Обновлять, только если `parseInt(remote) > parseInt(local)` (`index.html` boot + `app.js` `isNewerBuild`) |
| Кнопка | По-прежнему сбрасывает Cache Storage + unregister SW и перезагружает; после фикса Pages даёт свежую сборку |

### 3.18. Отчёты — диаграмма подрядчиков (web-253)

Диаграмма нарушений по подрядчикам строится по **тем же фильтрам**, что остальной дашборд (год, объект, акт, подрядчик), а не по всем актам.

### 3.19. Реестр нарушений на телефоне (web-258)

**Проблема:** Настройки → Нарушения → выбор реестра → тап в строку поиска. Safari зумит страницу, потому что у `.form-control` было **13–15px** (порог iOS — **16px**). После зума интерфейс остаётся увеличенным и не влазит в экран.

**Исправление:**

- На `gazprom-mobile-shell` у `input` / `select` / `textarea` — **16px** (в т.ч. `#vrScreenSearch`, `#vtSearch`, справочники, история).
- `-webkit-text-size-adjust: 100%` на `html`.
- Экран `#screen-violations`: поиск и фильтр **столбиком** на всю ширину, список — **карточки** (формулировка, вид, примечание, кнопки 44px), без скрытия колонок.
...
| web-125 | **Модалка нарушения:** прокрутка `.modal-body` на телефоне; список реестра не сужается при клавиатуре; `textarea` с max-height |
| web-122 | **Все экраны:** mobile shell на **flex** (не grid); исправлены safe-area padding у шапки и `.main` |
| web-217 | **Справка:** объекты проверки из каталога (чипы), детали объектов; стабильный импорт бэкапа; ML-мастер — читаемый выбор нарушения |
| web-219 | **Кроссбраузерность:** file picker на мобильных, стабильность выбора файлов |
| web-221 | **Справка:** шаблон Word в настройках; общий UI нарушений (`wizard-violations-ui.js`); субподрядчики/работники из каталога организаций; lightbox фото |
| web-222 | **Справка:** переключатели «Место нарушения» и «Пункт правил (в скобках)» в тексте Word; `SpravkaUtils.formatViolationText()`; превью в карточках |
| web-241 | **Главная:** статистика нарушений вместо длинной подписи на титуле |
| web-242 | **Мастер:** «Назад/Далее» временно `position: fixed` над баром — **откат в web-246** |
| web-243 | **Справочники:** комиссия / орг. / объекты / представители — fullscreen-карточки на телефоне; длинные имена в колонку |
| web-244 | **Главная:** убрана лишняя строка под приветствием |
| web-246 | Справочники: «Добавить» и «Отмена» одного размера; **мастер:** кнопки снова в потоке документа |
| web-247 | Сброс кэша PWA после правки прокрутки мастера |
| web-250 | **Выводы комиссии:** поле не `readonly` после шаблона — сразу правится |
| web-251 | **Скролл:** `.main` block, экран растёт по контенту, низ меню долистывается выше бара |
| web-252 | **Нарушения в акте:** поиск в одну строку на телефоне |
| web-253 | **Отчёты:** диаграмма по подрядчикам учитывает фильтры дашборда |
| web-254 | **PWA:** деплой всегда с `main`; обновление только если сборка **новее** (фикс отката 253→244) |
| web-255 | **Скролл:** один зазор = высота бара + 8px; спейсер 0; без запаса 48+24px |
| web-258 | **Реестр нарушений:** 16px в полях (без iOS-zoom); список карточками на телефоне |
...
2. **`git push origin main`** — иначе на GitHub Pages останется старая сборка. Не ждать отдельного «задеплой».
3. **Обновить этот файл** — строка сборки в шапке документа, нужные §3.*, таблица §7, §8, §10.
4. Убедиться, что Actions задеплоил **текущий** `main` (не старый SHA). На главной PWA — номер **новее** предыдущего.

---

## 8. Чеклист проверки на телефоне

1. **Главная** → внизу по центру актуальная «Сборка: web-255» (или новее); метка **не прыгает** при обновлении страницы; на **мастере** подписи сборки **нет**; под приветствием **нет** лишней строки (web-244).
...
7. **Назад / Далее** — в одну строку, равная ширина; **не fixed**; при прокрутке до конца стоят **сразу над** нижним баром, без большой пустой полосы (web-246, web-251, web-255). На последнем шаге «💾 Сохранить черновик».
...
10. Нижний бар — **6 пунктов** (в т.ч. **Отчёты**); иконки **28 px**, не меняют размер при tap; активный пункт — **только голубое подчёркивание** снизу (§3.10); контент долистывается **выше** бара (~8px зазор).
...
16. **Справка** — шаг 4: переключатели формата нарушений; карточки отражают итоговый текст; генерация Word с местом и пунктом правил (web-222).
17. **Настройки → Шаблоны Word** — разделы «Акт» и «Справка по ПБ»; статус плитки `акт` / `справка` / `акт+справка` (web-221).
18. **Справочники** (комиссия, орг., объекты, представители) — fullscreen-карточки; «Добавить» и «Отмена» одного размера (web-243, web-246).
19. **Выводы комиссии** — после шаблона поле редактируется сразу (web-250).
20. **Настройки → Обновить приложение** — ставит только **более новую** сборку, не откатывает (web-254).
21. **Отчёты** — диаграмма подрядчиков меняется вместе с фильтрами (web-253).
...
- [x] Мастер справки: объекты, субподрядчики, работники, нарушения — web-217, web-221
- [x] Шаблон Word справки в настройках — web-221
- [x] Формат нарушений в справке (место + пункт в скобках) — web-222
- [x] Справочники комиссии/орг./объектов/представителей на телефоне — web-243, web-246
- [x] «Назад/Далее» в потоке, скролл выше бара, узкий зазор — web-246, web-251, web-255
- [x] Выводы комиссии редактируются после шаблона — web-250
- [x] Поиск нарушений в одну строку — web-252
- [x] Отчёты: фильтры в диаграмме подрядчиков — web-253
- [x] Реестр нарушений: без iOS-zoom, карточки на телефоне — web-258
...
_Обновлено 26.08.2026 под **web-258**. При следующем релизе — синхронизировать `GAZPROM_ASSET_V`, нужные §3.*, таблицу §7, §8, §10._
