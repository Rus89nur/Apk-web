...
const GazpromStore = (() => {
  ...
  async function set(data, { skipPhotoIngest = true, verifyWrite = false, keepDraft = false } = {}) {
    ...
    if (!verified) {
      cache = null;
      throw new Error(
        'Данные не сохранились в браузере после импорта. Выйдите из режима инкогнито / «частной сессии», освободите память или используйте «Вставить текст» для небольшой копии.'
      );
    }
    cache = fromDb;
  }
